...
import { sendResetCodeEmail } from "@/app/lib/email";
...
export async function POST(req: NextRequest) {
  ...
  try {
    await sendResetCodeEmail({ to: email, code, locale });
  } catch (e) {
    // si falla email, no revelamos; pero devolvemos 200 igualmente
    console.error("sendResetCodeEmail failed:", e);
  }
